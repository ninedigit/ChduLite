    ...
    try
    {
        switch (action)
        {
            ...
            case 3:
                ...
                var printResult = await chdu.WriteAndPrintBlockAsync(blockToPrint, cancellationToken);
                var writtenBlock = await chdu.ReadBlockAsync(printResult.Address, cancellationToken);
                
                string printedData;
                
                if (writtenBlock.Content is BlockContent)
                    printedData = encoding.GetString(writtenBlock.Content.ToArray());
                else if (writtenBlock.Content is OffsettedBlockContent blockContent)
                    printedData = encoding.GetString(blockContent.ToArray()).Substring(blockContent.Offset);
                else
                    throw new NotImplementedException();
                
                logger.LogInformation("Following message was printed: {PrintedData}", printedData);
                break;
            case 4:
                var blockAddrToRead = (await chdu.GetStatusAsync(cancellationToken)).GetUsedBlocksCount() - 1;
                if (blockAddrToRead > 0)
                {
